...
public class ProcessRequestServiceImpl implements ProcessRequestService {
	...
	public String trackRequest(String id,String token) {
		...
		String url = "http://localhost:8080/component/componentprocessingmodule/trackProcessing/{requestId}";
		ResponseEntity<ProcessDTO> dto = restTemplate.postForEntity(url, entity, ProcessDTO.class,id);
		return "Your delivery date:"+dto.getBody().getDateOfDelivery();
	}

	@Override
	public String cancelRequest(String id, String cardNumber, String token) {
		HttpEntity<String> entity=template.getEntity(token);
		RestTemplate restTemplate = new RestTemplate();
		String url = "http://localhost:8080/component/componentprocessingmodule//cancelProcessing/{requestId}/{creditCardNumber}";
		ResponseEntity<String> dto = restTemplate.postForEntity(url, entity, String.class,id,cardNumber);
		return dto.getBody();
	}
